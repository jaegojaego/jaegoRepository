...
import org.springframework.ui.Model;   //수정시 import 권록헌
...
public class SellerController {
 ...
 //수정 20170405권록헌
    ...
    public String sellerupdate(String sellerId,Model model) {
       Seller seller=dao.selectOne(sellerId);
    ...
    public String rgoodsupdate(Seller seller,HttpSession session,MultipartFile upload,Model model){
       ...
       System.out.println("업데이트"+seller);
       //기존에첨부파일있다면 기존에파일을지우는것
       //새로파일을 작성
       String id=seller.getSellerId();
       String savefile =dao.selectOne(id).getSellerShopSimg();
       if(!upload.isEmpty()){
    ...
    public void download(String sellerId,HttpServletResponse response){
       Seller seller=dao.selectPic1(sellerId);
       try {
